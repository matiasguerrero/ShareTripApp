...
const CustomCalendar = ({ maxMonthsToRender, setToggleModal, setDate }) => {
  ...
  const [isFetchingAdditionalMonths, setIsFetchingAdditionalMonths] =
    useState(false)
  ...
  const handleScroll = (event) => {
    if (event.nativeEvent) {
      ...
      if (
        scrollPosition + scrollViewHeight >= contentHeight &&
        !isLoading &&
        !isFetchingAdditionalMonths
      ) {
  ...
  const renderMonth = (monthIndex) => {
    ...
    return (
      <View
        key={monthKey}
        style={[{ marginBottom: 0 }, { backgroundColor: "#131514" }]}
      >
        <Calendar
          current={monthKey}
          hideExtraDays
